...
import { Movie, People, RootMovies, RootPeople } from '../sw-interfaces';
...
export class FilmPeopleService {
  ...
  getFilmsByName(title: string) {
    return this.http.get<RootMovies>(this.movieUrl + `/?title=${title}`)
      .pipe(
  ...
  getSwPeopleByUrl(url: string) {
    ...
    return this.http.get<RootPeople>(`api/swPeople/${id}`)
      .pipe(
