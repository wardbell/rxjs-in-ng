...
const serialize = o =>
    Object.keys(o).reduce(
        (search, key) => (search += `${key}=${encodeURIComponent(o[key])}&`),
        ''
    );
...
export class RakiService {
    ...
    detail$: Observable<
        RakiObject.ArtDetailObject[]
    > = this.detailNumber.switchMap(
        number =>
            number
                ? this.http
    ...
    randomImage$ = this.http
        ...
        .switchMap(artObject =>
            Observable.fromPromise(this.preload(artObject.webImage.url))
        );
    ...
    private getArtObject$: Observable<ArtObject> = Observable.create(obs => {
        ...
        .switchMap(selection =>
            this.http
                ...
                .catch((e): Observable<ArtObject> =>
                    Observable.timer(500).switchMap(
                        (): Observable<ArtObject> => this.getArtObject$
                    )
                )
        )
        .switchMap(
