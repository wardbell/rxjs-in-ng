...
import {
  catchError, concat, distinctUntilChanged,
  map, merge, mergeMap,
  scan, shareReplay, startWith, switchMap, tap } from 'rxjs/operators';
...
export class SwPeopleService {
  ...
  people$ = this._people$.pipe(
      mergeMap(list => this.additions$.pipe(
      map(additions => list.concat(additions)))
    ),
    map(list => list.sort((x, y) => (x.name < y.name ? -1 : 1)))
  );

  loading$ = of(true).pipe(
    merge(this._people$.pipe(
      map(_ => true),
      concat(of(false)))
    ),
